  ...
  <section id="gallery" class="relative py-24 md:py-32 overflow-hidden bg-navy">
    ...
    <Transition name="lightbox">
      <div v-if="lightboxOpen" class="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-4"
        ...
        <button class="absolute top-6 right-6 text-white/70 hover:text-gold text-2xl transition-colors"
          @click="lightboxOpen = false">
          ✕
        </button>
        <img :src="activeImage" :alt="activeAlt" class="max-h-[85vh] max-w-full object-contain"
          style="border: 1px solid var(--gold-30)" @click.stop />
      </div>
    </Transition>
    ...
    <div class="max-w-6xl mx-auto px-6">
      ...
      <div class="text-center mb-16">
        <p class="font-sans text-gold/70 text-xs tracking-[0.5em] uppercase mb-4 reveal">
          {{ invitation.gallery.sectionLabel }}
        </p>
      ...
      <div class="columns-2 md:columns-3 gap-4 space-y-4">
        <div v-for="(photo, index) in photos" :key="photo.url"
          ...
          style="border: 1px solid rgb(var(--color-gold) / 0)" @click="openLightbox(photo)">
          ...
          <div
            class="absolute inset-0 border border-gold/0 group-hover:border-gold/50 transition-all duration-500 pointer-events-none" />
        </div>
      </div>
    </div>
  </section>
...
const photos: Photo[] = invitation.gallery.photos
